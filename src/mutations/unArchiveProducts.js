...
export default async function unArchiveProducts(context, input) {
  ...
  const productIdsToUnArchive = productsWithVariants.map((product) => product._id);

    const unArchivedProducts = await Promise.all(productIdsToUnArchive.map(async (productId) => {
  ...
  return unArchivedProducts.filter((unArchivedProduct) => productIds.includes(unArchivedProduct._id));
